...
for filepath in newFiles:
	...
	if os.path.isfile(o):
		continue
	ffmpeg = 'D:\Chris\\Documents\\exe files\\ffmpeg-6.0-essentials_build\\bin\\ffmpeg.exe'
	encodeCommand = '"%s" -i "%s" -vf scale=-2:480 -c:v libx264 -profile:v baseline -level 3.0 -preset fast -crf 23 -pix_fmt yuv420p "%s"' % (ffmpeg, i, o)
